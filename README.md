...
项目根目录下提供 `start-all.ps1` 脚本，可一条命令同时启动后端和前端开发环境。
...
前端支持多环境 API 地址配置。请在 `frontend` 目录下新建 `.env.development`、`.env.production` 等文件，内容如下：

```env
VITE_API_BASE_URL=http://localhost:8080/api
```

在代码中通过 `import.meta.env.VITE_API_BASE_URL` 获取后端 API 地址。
...
前端生产环境建议用 Nginx 代理后端 API，实现前后端分离部署。示例配置如下：
...
server {
    ...
    location / {
        root /usr/share/nginx/html; # 前端dist目录
...
> 详细部署方案请参考 [docs/deployment.md](docs/deployment.md)
